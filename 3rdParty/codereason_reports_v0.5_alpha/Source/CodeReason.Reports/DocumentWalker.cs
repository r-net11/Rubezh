...
namespace CodeReason.Reports
{
	...
	public class DocumentWalker
	{
		...
		public List<T> TraverseRowGroup<T>(TableRowGroup trg) where T : class
		{
			List<T> res = new List<T>();
			if (VisualVisited != null)
				VisualVisited(this, trg, true);
			foreach (TableRow tr in trg.Rows)
			{
				if (VisualVisited != null)
					VisualVisited(this, tr, true);
				if (tr is T)
					res.Add(tr as T);
			...
			foreach (Block b in blocks)
			{
				...
				if (t != null)
				{
					...
					foreach (TableRowGroup trg in t.RowGroups)
						res.AddRange(TraverseRowGroup<T>(trg));
					continue;
				}
			}
			return res;
