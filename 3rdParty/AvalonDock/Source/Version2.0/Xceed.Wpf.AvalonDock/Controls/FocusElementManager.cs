	...
	internal static class FocusElementManager
	{
		...
		static void manager_PreviewGotKeyboardFocus(object sender, KeyboardFocusChangedEventArgs e)
		{
			...
			if (focusedElement != null &&
				!(focusedElement is LayoutAnchorableTabItem || focusedElement is LayoutDocumentTabItem || focusedElement is TabItem))
		...
		internal static void SetFocusOnLastElement(ILayoutElement model)
		{
			...
			if (!focused)
			{
				var win = model.Root.Manager.FindVisualAncestor<Window>();
				if (win != null && !win.IsActive)
				{
					var focusedObj = FocusManager.GetFocusedElement(win) as TabItem;
					if (focusedObj != null && !focusedObj.IsSelected)
					{
						var tabControl = focusedObj.FindVisualAncestor<TabControl>();
						if (tabControl != null)
						{
							//var tabItem = tabControl.FindVisualChildren<TabItem>().FirstOrDefault(item => item.IsSelected);
							var tabItem = tabControl.ItemContainerGenerator.ContainerFromItem(tabControl.SelectedContent) as UIElement;
							if (tabItem != null)
								tabItem.Focus();
						}
					}
		...
		internal static void ClearLastElement(ILayoutElement model)
		{
			_modelFocusedElement.ClearValue(model);
		}
