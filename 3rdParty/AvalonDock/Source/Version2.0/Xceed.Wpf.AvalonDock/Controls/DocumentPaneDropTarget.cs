...
namespace Xceed.Wpf.AvalonDock.Controls
{
	...
	{
		private double _tabItemHeight;
		private int _tabIndex = -1;

		internal DocumentPaneDropTarget(LayoutDocumentPaneControl paneControl, Rect detectionRect, DropTargetType type)
			: base(paneControl, detectionRect, type)
		{
			var tabItem = TargetElement.FindVisualChildren<LayoutDocumentTabItem>().FirstOrDefault();
			if (tabItem != null)
				_tabItemHeight = tabItem.GetScreenArea().Height;
		}
		...
		{
			ILayoutDocumentPane targetModel = TargetElement.Model as ILayoutDocumentPane;
				...
				case DropTargetType.DocumentPaneDockInside:
					{
						...
						if (_tabIndex == -1)
						{
							if (TargetElement.Items.Count > 1 && floatingWindowModel is LayoutDocumentFloatingWindow)
							{
								targetScreenRect.Offset(targetScreenRect.Width / 2.0, _tabItemHeight);
								targetScreenRect.Width /= 2.0;
								targetScreenRect.Height -= _tabItemHeight;
