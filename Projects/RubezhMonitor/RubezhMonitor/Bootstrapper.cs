...
namespace RubezhMonitor
{
	public class Bootstrapper : BaseBootstrapper
	{
			...
			if (ServiceFactory.StartupService.PerformLogin(Login, Password))
			{
				...
				try
				{
					...
					ProcedureExecutionContext.Initialize(
						...
						SafeRubezhService.ProcessAutomationCallback,
						((SafeRubezhService)ClientManager.RubezhService).ProcedureCallbackResponse,
						OnSynchronizeVariable,
						((SafeRubezhService)ClientManager.RubezhService).AddJournalItem,
						((SafeRubezhService)ClientManager.RubezhService).ControlGKDevice,
						((SafeRubezhService)ClientManager.RubezhService).StartRecord,
						((SafeRubezhService)ClientManager.RubezhService).StopRecord,
						((SafeRubezhService)ClientManager.RubezhService).Ptz,
						((SafeRubezhService)ClientManager.RubezhService).RviAlarm,
						((SafeRubezhService)ClientManager.RubezhService).RviOpenWindow,
						((SafeRubezhService)ClientManager.RubezhService).ControlFireZone,
						((SafeRubezhService)ClientManager.RubezhService).ControlGuardZone,
						((SafeRubezhService)ClientManager.RubezhService).ControlDirection,
						((SafeRubezhService)ClientManager.RubezhService).ControlGKDoor,
						((SafeRubezhService)ClientManager.RubezhService).ControlDelay,
						((SafeRubezhService)ClientManager.RubezhService).ControlPumpStation,
						((SafeRubezhService)ClientManager.RubezhService).ControlMPT,
						((SafeRubezhService)ClientManager.RubezhService).ExportJournal,
						((SafeRubezhService)ClientManager.RubezhService).ExportOrganisation,
						((SafeRubezhService)ClientManager.RubezhService).ExportOrganisationList,
						((SafeRubezhService)ClientManager.RubezhService).ExportConfiguration,
						((SafeRubezhService)ClientManager.RubezhService).ImportOrganisation,
						((SafeRubezhService)ClientManager.RubezhService).ImportOrganisationList,
						GetOrganisations,
						() => { return ClientManager.CurrentUser; },
						() => { return RubezhServiceFactory.UID; }
						);
					...
					result = Run();
					SafeRubezhService.ConfigurationChangedEvent += () => { ApplicationService.Invoke(OnConfigurationChanged); };
					SafeRubezhService.RestartEvent += () => { ApplicationService.Invoke(Restart); };
					...
					SafeRubezhService.AutomationEvent -= OnAutomationCallback;
					SafeRubezhService.AutomationEvent += OnAutomationCallback;
					...
					SafeRubezhService.JournalItemsEvent += OnJournalItems;

					ScheduleRunner.Start();

					AutomationProcessor.RunOnApplicationRun(ClientManager.SecurityConfiguration.Users.FirstOrDefault(x => x.Login == ClientManager.ClientCredentials.Login), RubezhServiceFactory.UID);
					//MutexHelper.KeepAlive();
					if (Process.GetCurrentProcess().ProcessName != "RubezhMonitor.vshost")
					{
		...
		{
			((SafeRubezhService)ClientManager.RubezhService).WriteOpcDaServerTag(RubezhServiceFactory.UID, tagUID, value);
		}
		...
		{
			var properties = ClientManager.RubezhService.GetProperties(Guid.Empty);
			if (properties != null)
			{
		...
		{
			var result = ClientManager.RubezhService.GetOrganisations(new RubezhAPI.SKD.OrganisationFilter());
		...
		private void OnJournalItems(List<JournalItem> journalItems, bool isNew)
		{
			if (isNew)
				foreach (var journalItem in journalItems)
					AutomationProcessor.RunOnJournal(journalItem, ClientManager.CurrentUser, RubezhServiceFactory.UID);
		}
		...
		public void Restart(string login = null, string password = null, bool isNotRestart = false)
		{
			using (new WaitWrapper())
			{
				...
				ClientManager.RubezhService.StopPoll();
				LoadingErrorManager.Clear();
