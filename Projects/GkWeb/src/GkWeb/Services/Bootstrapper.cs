...
namespace GkWeb.Services
{
	...
	{
		private object syncBootstrapper = new object();
		...
		void Run() {
			...
			for (int i = 1; i <= 10; i++) {
				var message = RubezhClient.ClientManager.Connect(RubezhAPI.Models.ClientType.WebService, ConnectionSettingsManager.ServerAddress,
		...
		private void SafeFiresecServiceOnConfigurationChangedEvent() {
			InitServer();
		...
		private void GKPropertyChangedEvent(GKPropertyChangedCallback obj) {
		...
		void CopyDeviceMeasureParametersStates(GKStates gkStates) {
		...
		void OnNewJournalItem(List<JournalItem> journalItems, bool boool) {
