...
namespace RubezhService.Models
{
	static class PollingModel
	{
		...
		{
			var clientInfo = RubezhService.Service.ClientsManager.ClientInfos.FirstOrDefault(x => x.UID == clientUID);
			var pollingItem = PollingItems.FirstOrDefault(x => x.ClientUID == clientUID);
			var connection = ConnectionsModel.Connections.FirstOrDefault(x => x.UID == clientUID);
