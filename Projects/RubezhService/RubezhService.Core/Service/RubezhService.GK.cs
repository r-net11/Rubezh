	...
	public partial class RubezhService
	{
		...
		public OperationResult<bool> GKSetSingleParameter(Guid clientUID, Guid objectUID, List<byte> parameterBytes, List<GKProperty> deviceProperties)
		{
			...
			if (gkBase != null)
			{
				...
				{
					RubezhService.NotifyGKParameterChanged(objectUID, deviceProperties, clientUID);
				}
				return result;
			}
				...
				ServerTaskRunner.Add(progressCallback, "Перезапись пользователей прибора", new Action(() =>
					{
						...
						try
						{
							...
							RubezhService.NotifyOperationResult_RewriteUsers(new OperationResult<bool>(true), clientUID);
						}
						catch (Exception e)
						{
							RubezhService.NotifyOperationResult_RewriteUsers(OperationResult<bool>.FromError(e.Message), clientUID);
						}
						finally
						{
							GKProcessorManager.StopProgress(progressCallback);
