...
public class LicenseView
{
	...
	private void LoadLicenseButtonOnClicked(object sender, EventArgs eventArgs)
	{
		...
		try
		{
			File.Copy(filePathEntry.Text, AppDataFolderHelper.GetFile("RubezhService.license"), true);
		}
		catch (Exception e)
		{
		...
		RubezhLicenseProcessor.SetLicense(LicenseManager.TryLoad(AppDataFolderHelper.GetFile("RubezhService.license")));
	}
