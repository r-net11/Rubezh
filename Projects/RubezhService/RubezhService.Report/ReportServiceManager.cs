...
namespace RubezhService.Report
{
	public class ReportServiceManager
	{
		...
		void Open()
		{
				...
				_serviceHost = new ServiceHost(typeof(RubezhReportService));
		...
		{
			ServiceKnownTypeProvider.Register(typeof(IRubezhService).Assembly.GetTypes().Where(t => t.IsClass && !t.IsAbstract && t.Namespace == FilterNamespace));
		}
	}
}