...
namespace ServerFS2
{
	public static partial class ServerHelper
	{
		...
		static void ParseUIDevicesFlash(ref int pointer, DriverType driverType)
		{
			...
			{
				case DriverType.MPT:
                    device.InnerDeviceParameters = CreateBytesArray(DeviceFlash[pointer + 31], DeviceFlash[pointer + 32], DeviceFlash[pointer + 33]);
                    break;

				case DriverType.MDU:
                    device.InnerDeviceParameters = CreateBytesArray(DeviceFlash[pointer + 31], DeviceFlash[pointer + 32]);
            ...
            if (driverType == DriverType.MRO_2)
            {
                if (parentAddress > 0x100)
                {
                    ...
                    if (groupDevice == null) // если такое ГУ ещё не добавлено
                    {
                        ...
                        {
                            Driver = Drivers.FirstOrDefault(x => x.DriverType == DriverType.MRO_2),
			...
			if (driverType == DriverType.RM_1)
			{
				if ((config&16)>>4 == 1)
				{
					...
					{
						case 2:
					        groupDevice.Driver = Drivers.FirstOrDefault(x => x.DriverType == DriverType.RM_2);
                            groupDevice.DriverUID = groupDevice.Driver.UID;
							break;
						case 3:
					        groupDevice.Driver = Drivers.FirstOrDefault(x => x.DriverType == DriverType.RM_3);
                            groupDevice.DriverUID = groupDevice.Driver.UID;
							break;
						case 4:
                            groupDevice.Driver = Drivers.FirstOrDefault(x => x.DriverType == DriverType.RM_4);
                            groupDevice.DriverUID = groupDevice.Driver.UID;
							break;
						case 5:
					        groupDevice.Driver = Drivers.FirstOrDefault(x => x.DriverType == DriverType.RM_5);
                            groupDevice.DriverUID = groupDevice.Driver.UID;
							break;
					}
					return;
			...
			if (driverType == DriverType.MPT)
			{
                ...
                if (parentAddress > 0x100)
                {
                    ...
                    if (groupDevice == null) // если такое ГУ ещё не добавлено
                    {
                        groupDevice = new Device
                        {
                            Driver = Drivers.FirstOrDefault(x => x.DriverType == DriverType.MPT),
                            IntAddress = parentAddress
                        };
                        groupDevice.DriverUID = groupDevice.Driver.UID;
                        PanelDevice.Children.Add(groupDevice);
        ...
        static void ParseNoUIDevicesFlash(ref int pointer, DriverType driverType)
        {
            ...
            if (driverType == DriverType.AM_1)
            {
                ...
                if ((config&16)>>4 == 1)
                {
                    ...
                    if (groupDevice == null) // если такое ГУ ещё не добавлено
                    {
                        groupDevice = new Device
                        {
                            Driver = Drivers.FirstOrDefault(x => x.DriverType == DriverType.AM4),
                            IntAddress = device.IntAddress - localNo
                        };
                        groupDevice.DriverUID = groupDevice.Driver.UID;
                        PanelDevice.Children.Add(groupDevice);
            ...
            if (driverType == DriverType.AMP_4)
            {
                ...
                if (groupDevice == null) // если такое ГУ ещё не добавлено
                {
                    groupDevice = new Device
                    {
                        Driver = Drivers.FirstOrDefault(x => x.DriverType == DriverType.AM4_P),
                        IntAddress = device.IntAddress - localNo
                    };
                    groupDevice.DriverUID = groupDevice.Driver.UID;
                    PanelDevice.Children.Add(groupDevice);
            ...
            if((driverType == DriverType.RadioHandDetector)||(driverType == DriverType.RadioSmokeDetector))
            {
                ...
                if (groupDevice == null)
                {
                    groupDevice = new Device
                    {
                        Driver = Drivers.FirstOrDefault(x => x.DriverType == DriverType.MRK_30),
