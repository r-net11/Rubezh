...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="wix:Wix">
    ...
    <Wix>
      <Fragment>
        <DirectoryRef Id="Output.ReportTemplates">
          <xsl:apply-templates select="$reportTemplates"/>
        </DirectoryRef>
      </Fragment>

      <Fragment>
        <ComponentGroup Id="Output.ReportTemplates">
  ...
  <xsl:template match="wix:Component">
    <Component Id="{@Id}" Guid="{@Guid}">
      <File Id="{wix:File/@Id}" Source="{concat('..\..\source\Configuration\ReportTemplates', substring(wix:File/@Source, 10))}" />
    </Component>
  </xsl:template>
