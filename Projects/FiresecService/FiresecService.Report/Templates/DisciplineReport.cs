				...
				if (timeTrackEmployeeResult != null)
				{
					foreach (var dayTimeTrack in timeTrackEmployeeResult.DayTimeTracks)
					{
						...
						if (dayTimeTrack.RealTimeTrackParts.Count > 0)
						{
							...
							dataRow.LastExit = dayTimeTrack.RealTimeTrackParts.Where(x => x.ExitDateTime.HasValue && x.IsForURVZone).Select(x => x.ExitDateTime).DefaultIfEmpty().Max().GetValueOrDefault().TimeOfDay;
						}
