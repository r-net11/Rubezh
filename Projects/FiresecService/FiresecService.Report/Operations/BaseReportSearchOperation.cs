...
{
	/// <summary>
	/// Базовый класс реализации поиска данных для формирования шаблонов пропусков, при отправке на печать.
	/// </summary>
	public abstract class BaseReportSearchOperation
	{
		...
		/// <summary>
		/// Ищет в БД все данные для шаблона пропуска, относящиеся к текущему сотруднику.
		/// </summary>
		/// <param name="employee">Входной сотрудник.</param>
		/// <returns>Объект, отвечающий за передачу необходимой информации о сотруднике для формирования шаблона пропуска.</returns>
		protected CardTemplatePrintDataDTO CreateDataSetItem(Employee employee)
		{
			if (employee == null)
				throw new ArgumentNullException("employee");
			if(employee.Cards == null)
				throw new ArgumentNullException("employee.cards");
