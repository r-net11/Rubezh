...
using DataClasses = SKDDriver.DataClasses;
...
namespace FiresecService.Service
{
    public partial class FiresecService : IFiresecService
    {
        ...
        {
            using (var databaseService = new DataClasses.DbService())
            {
        ...
        {
            return new OperationResult();// BeginGetAsync<EmployeeFilter, DataClasses.EmployeeAsyncTranslator>(filter, uid, DataClasses.EmployeeAsyncTranslator.CurrentThread);
        }
        #endregion
        ...
        IEnumerable<string> AddStrazhCard(SKDCard card, AccessTemplate accessTemplate, SKDDriver.DataClasses.DbService databaseService)
        {
        ...
        IEnumerable<string> EditStrazhCard(SKDCard oldCard, AccessTemplate oldAccessTemplate, SKDCard card, AccessTemplate accessTemplate, SKDDriver.DataClasses.DbService databaseService)
        {
        ...
        {
            if (newCard.EmployeeUID != null)
            {
                var employeeOperationResult = databaseService.EmployeeTranslator.GetSingle(newCard.EmployeeUID.Value);
                if (!employeeOperationResult.HasError)
                {
        ...
        IEnumerable<string> DeleteStrazhCard(SKDCard card, AccessTemplate accessTemplate, SKDDriver.DataClasses.DbService databaseService)
        {
        ...
        {
            return new OperationResult();// return BeginGetAsync<CardFilter, DataClasses.CardAsyncTranslator>(filter, uid, DataClasses.CardAsyncTranslator.CurrentThread);
        }
        #endregion
        ...
        public OperationResult MarkDeletedOrganisation(Guid uid, string name)
        {
            ...
            {
                var errors = new List<string>();
                var cards = dbService.CardTranslator.Get(
                    new CardFilter { EmployeeFilter = new EmployeeFilter { OrganisationUIDs = new List<Guid> { uid } }, DeactivationType = LogicalDeletationType.Active });
                if (!cards.HasError)
                {
        ...
        //public OperationResult BeginGetAsync<TFilter, TTranslator>(TFilter filter, Guid uid, Thread currentThread)
        //    where TTranslator : class, IAsyncTranslator<TFilter>
        //{
        //    try
        //    {
        //        if (currentThread != null)
        //        {
        //            DataClasses.DbService.IsAbort = true;
        //            currentThread.Join(TimeSpan.FromMinutes(1));
        //            currentThread = null;
        //        }
        //        DataClasses.DbService.IsAbort = false;
        //        var thread = new Thread(new ThreadStart((new Action(() =>
        //        {
        //            using (var dbService = new SKDDriver.DataClasses.DbService())
        //            {
        //                var translator = dbService.GetTranslator<TTranslator>();
        //                translator.PortionReady -= DatabaseHelper_DbPortionReady;
        //                translator.PortionReady += DatabaseHelper_DbPortionReady;
        //                translator.BeginGet(filter, uid);
        //            }
        //        }))));
