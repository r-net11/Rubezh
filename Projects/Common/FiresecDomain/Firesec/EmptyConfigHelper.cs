...
namespace FiresecDomain
{
	public static class EmptyConfigHelper
	{
		...
		{
			string defaultConfigString = Firesec.Properties.Settings.Default.DefaultConfig;
            if (string.IsNullOrEmpty(defaultConfigString))
				return new OperationResult<Firesec.Models.CoreConfiguration.config>("Нулевая конфтгурация по умолчанию");
