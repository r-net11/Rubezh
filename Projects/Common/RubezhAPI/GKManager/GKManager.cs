...
namespace RubezhAPI
{
    public partial class GKManager
    {
        ...
        public static void AddReservedGkDevices()
        {
            var gkDevices = Devices.FindAll(x => x.DriverType == GKDriverType.GK && !String.IsNullOrEmpty(x.GetReservedIpAddress()));
            foreach (var gkDeivice in gkDevices)
            {
                var reservedIp = gkDeivice.GetReservedIpAddress();
                var reservedGkDevice = CopyDevice(gkDeivice, false);
                reservedGkDevice.Children.RemoveAll(x => x.Driver.IsKau);
                DeviceConfiguration.ReservedDevices.AddRange(reservedGkDevice.AllChildrenAndSelf);
                var ipProperty = reservedGkDevice.Properties.FirstOrDefault(x => x.Name == "IPAddress");
                if (ipProperty != null)
                    ipProperty.StringValue = reservedIp;
            }
        }
        ...
        public static string GetIpAddress(GKDevice device, bool useReserveIp = false)
        {
            ...
            string ipAddress;
            if (useReserveIp)
                ipAddress = gkControllerDevice.GetReservedIpAddress();
            else
                ipAddress = gkControllerDevice.GetMainIpAddress();
            return ipAddress;
        }
