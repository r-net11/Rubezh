...
namespace Common.GK
{
    public class MPT_Helper
    {
        public static XDriver Create()
        {
            ...
            GKDriversHelper.AddIntProprety(driver, 0xAB, "время включенного состояния выхода 1", 0, 2, 0, 255).IsLowByte = true;
            GKDriversHelper.AddIntProprety(driver, 0xAC, "время включенного состояния выхода 2", 0, 2, 0, 255).IsLowByte = true;
            GKDriversHelper.AddIntProprety(driver, 0xAD, "время включенного состояния выхода 3", 0, 2, 0, 255).IsLowByte = true;
            GKDriversHelper.AddIntProprety(driver, 0xAE, "время включенного состояния выхода 4", 0, 2, 0, 255).IsLowByte = true;
            GKDriversHelper.AddIntProprety(driver, 0xAF, "время включенного состояния выхода 5", 0, 2, 0, 255).IsLowByte = true;

            GKDriversHelper.AddIntProprety(driver, 0xB1, "период переключения выхода 1", 0, 1, 0, 255).IsLowByte = true;
            GKDriversHelper.AddIntProprety(driver, 0xB2, "период переключения выхода 2", 0, 1, 0, 255).IsLowByte = true;
            GKDriversHelper.AddIntProprety(driver, 0xB3, "период переключения выхода 3", 0, 1, 0, 255).IsLowByte = true;
            GKDriversHelper.AddIntProprety(driver, 0xB4, "период переключения выхода 4", 0, 1, 0, 255).IsLowByte = true;
            GKDriversHelper.AddIntProprety(driver, 0xB5, "период переключения выхода 5", 0, 1, 0, 255).IsLowByte = true;

            GKDriversHelper.AddIntProprety(driver, 0xC1, "Время задержки включения выхода 1", 0, 3, 0, 255).IsLowByte = true;
            GKDriversHelper.AddIntProprety(driver, 0xC2, "Время задержки включения выхода 2", 0, 3, 0, 255).IsLowByte = true;
            GKDriversHelper.AddIntProprety(driver, 0xC3, "Время задержки включения выхода 3", 0, 3, 0, 255).IsLowByte = true;
            GKDriversHelper.AddIntProprety(driver, 0xC4, "Время задержки включения выхода 4", 0, 3, 0, 255).IsLowByte = true;
            GKDriversHelper.AddIntProprety(driver, 0xC5, "Время задержки включения выхода 5", 0, 60, 0, 255).IsLowByte = true;
            ...
            GKDriversHelper.AddPlainEnumProprety(driver, 0xC6, "Блокировка выключения режима «Автоматика включена» при неисправности", 2,
                ...
                "режим «Автоматика включена» не отключается при неисправности источника питания прибора, при неисправности ШС, при нарушении датчика «Двери-окна»", 1, 1).IsLowByte = true;
            ...
            GKDriversHelper.AddPlainEnumProprety(driver, 0xC6, "Восстановление режима «Автоматика включена»", 4,
                ...
                "режим не восстанавливается после восстановления  датчика «Двери-окна», восстановление возможно по протоколу RSR", 1, 2).IsLowByte = true;
