...
namespace GKProcessor
{
	public class MPTCreator
	{
		...
		public MPTCreator(GkDatabase gkDatabase, XMPT mpt)
		{
			...
			//foreach (var nsDevice in mpt.NSDevices)
			//{
		...
		void CreateDelays()
		{
			for (int i = 1; i < FirePumpDevices.Count; i++)
			{
				...
				var delay = new XDelay()
				{
					...
					//DelayTime = (ushort)MPT.NSDeltaTime,
					SetTime = 2,
		...
		void AddCountFirePumpDevicesFormula(FormulaBuilder formula)
		{
			...
			//formula.Add(FormulaOperationType.CONST, 0, (ushort)MPT.NSPumpsCount, "Количество основных пожарных насосов");
		...
		void CreatePim()
		{
			...
			Pim.UID = GuidHelper.CreateOn(MPT.BaseUID);
			...
			//inputDevices.AddRange(MPT.InputDevices);
			//foreach (var nsDevice in MPT.NSDevices)
			//{
		...
		void SetCrossReferences()
		{
			//foreach (var nsDevice in MPT.NSDevices)
			//{
			//    UpdateConfigurationHelper.LinkXBases(nsDevice, MPT);
			//}

			foreach (var pumpDelay in PumpDelays)
			{
				UpdateConfigurationHelper.LinkXBases(pumpDelay.Delay, MPT);
