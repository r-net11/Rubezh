        ...
        public static void Start()
        {
            ...
            foreach (var gkDatabase in DescriptorsManager.GkDatabases)
            {
                if (GKManager.IsValidIpAddress(gkDatabase.RootDevice))
                {
                    ...
                    watcher.StartThread();
                    if (GKManager.IsValidReservedIpAddress(gkDatabase.RootDevice))
                    {
                        watcher = new Watcher(gkDatabase, true);
                        Watchers.Add(watcher);
                        watcher.StartThread();
                        Watchers[Watchers.Count - 1].ReservedWatcher = Watchers[Watchers.Count - 2];
                        Watchers[Watchers.Count - 2].ReservedWatcher = Watchers[Watchers.Count - 1];
                    }
                }
            }
