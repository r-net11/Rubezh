	...
	public partial class FiresecManager
	{
		public static void SetZoneGuard(Zone zone)
		{
			try
			{
				if (IsFS2Enabled)
				{
					FS2ClientContract.SetZoneGuard(zone.UID);
				}
				else
				{
		...
		public static void UnSetZoneGuard(Zone zone)
		{
			try
			{
				if (IsFS2Enabled)
				{
					FS2ClientContract.UnSetZoneGuard(zone.UID);
				}
				else
				{
					if ((zone.ZoneType == ZoneType.Guard) && (zone.SecPanelUID != Guid.Empty))
		...
		public static void SetDeviceGuard(Device device)
		{
			try
			{
				if (IsFS2Enabled)
				{
					FS2ClientContract.SetDeviceGuard(device.UID);
				}
				else
				{
		...
		public static void UnSetDeviceGuard(Device device)
		{
			try
			{
				if (IsFS2Enabled)
				{
					FS2ClientContract.UnSetDeviceGuard(device.UID);
				}
				else
				{
