...
namespace SKDDriver.Translators
{
    public class PassJournalTranslator : IDisposable
    {
        ...
        public IEnumerable<DataAccess.PassJournal> GetEmployeesLastPassJournal(List<Guid> employeeUIDs, List<Guid> zoneUIDs, DateTime? dateTime)
        {
            var filter = PredicateBuilder.True<DataAccess.PassJournal>();
            if (!employeeUIDs.IsEmpty())
                filter.And(e => employeeUIDs.Contains(e.EmployeeUID));
            if (!zoneUIDs.IsEmpty())
                filter.And(e => zoneUIDs.Contains(e.ZoneUID));
            if (dateTime.HasValue)
                filter.And(e => e.EnterTime < dateTime && (!e.ExitTime.HasValue || e.ExitTime > dateTime));
            else
                filter.And(e => !e.ExitTime.HasValue);
            var uids = Context.PassJournals.Where(filter).GroupBy(item => item.EmployeeUID).Select(gr => gr.OrderByDescending(item => item.EnterTime).First().UID);
            return Context.PassJournals.Where(item => uids.Contains(item.UID));
        }
    }
}