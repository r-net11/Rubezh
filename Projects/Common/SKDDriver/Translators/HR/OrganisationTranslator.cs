...
namespace SKDDriver.DataClasses
{
	public class OrganisationTranslator
	{
		...
		{
			var command = string.Format("UPDATE {0} SET RemovalDate = '{1}', IsDeleted = 1 WHERE IsDeleted = 0 AND OrganisationUID = '{2}'", tableName, removalDate, organisationUID);
			Context.Database.ExecuteSqlCommand(command);
		}

		void RestoreSQLQuery(string tableName, DateTime removalDate, Guid organisationUID)
		{
			var command = string.Format("UPDATE {0} SET RemovalDate = NULL, IsDeleted = 0 WHERE IsDeleted = 1 AND OrganisationUID = '{2}' AND RemovalDate = '{1}'", tableName, removalDate, organisationUID);
			Context.Database.ExecuteSqlCommand(command);
		}
