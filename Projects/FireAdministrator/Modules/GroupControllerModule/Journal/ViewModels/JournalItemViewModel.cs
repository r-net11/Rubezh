	...
	public class JournalItemViewModel : BaseViewModel
	{
			...
			try
			{
				switch (JournalItem.JournalItemType)
				{
					...
					case XJournalItemType.PumpStation:
						var pumpStation = XManager.PumpStations.FirstOrDefault(x => x.BaseUID == JournalItem.ObjectUID);
						if (pumpStation != null)
						{
					...
					case XJournalItemType.Pim:
						var pim = XManager.AutoGeneratedPims.FirstOrDefault(x => x.BaseUID == JournalItem.ObjectUID);
						if (pim != null)
						{
		...
		void OnShowObject()
		{
			switch (JournalItem.JournalItemType)
			{
				...
				case XJournalItemType.PumpStation:
					if (XManager.PumpStations.Any(x => x.BaseUID == JournalItem.ObjectUID))
					{
