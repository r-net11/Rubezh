...
namespace RubezhDevicesMVVM
{
    class DevicesListViewModel : BaseViewModel
    {
        ...
        void OnAddCommand(object obj)
        {
            ...
            ViewModel.Current.DevicesViewModel.Add(deviceViewModel);
        ...
        public void LoadMetadata()
        {
            ...
            foreach (drvType item in metadata.drv)
                try
                {
                    if (item.options.Contains("Placeable")&&(ViewModel.Current.DevicesViewModel.FirstOrDefault(x => x.Id == item.name) == null))
                    {
                        DeviceViewModel deviceViewModel = new DeviceViewModel();
