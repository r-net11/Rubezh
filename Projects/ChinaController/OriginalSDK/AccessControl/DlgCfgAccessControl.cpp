...
void CDlgCfgAccessControl::InitDlg()
{
	...
	{
		m_cmbOpenMethod.InsertString(-1, ConvertString(stuDemoOpenMethod[i].szName, DLG_CFG_ACCESS_CONTROL));
	}
...
BOOL CDlgCfgAccessControl::SetConfigToDevice()
{
	...
	{		
		...
		{
			CString csErr;
			csErr.Format("%s %08x...", ConvertString("SetupConfig AccessControl failed:", DLG_CFG_ACCESS_CONTROL), CLIENT_GetLastError());
