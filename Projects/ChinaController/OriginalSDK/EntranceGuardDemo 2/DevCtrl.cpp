...
BOOL DevCtrl_CaptureFingerPrint(LLONG lLoginId)
{
    NET_CTRL_CAPTURE_FINGER_PRINT stuParam = {sizeof(stuParam)};
    stuParam.nChannelID = 0;
    strncpy(stuParam.szReaderID, "ReaderID", sizeof(stuParam.szReaderID) - 1);

    BOOL bRet = CLIENT_ControlDevice(lLoginId, DH_CTRL_CAPTURE_FINGER_PRINT, &stuParam, SDK_API_WAITTIME);
    if (bRet)
    {
        printf("Capture finger print at door %d with reader:%s ok!\n", stuParam.nChannelID, stuParam.szReaderID);
        return TRUE;
    }
    else
    {
        printf("Capture finger print at door %d with reader:%s failed:0x%08x\n", stuParam.nChannelID, stuParam.szReaderID, CLIENT_GetLastError());
        return FALSE;
    }
}

void DevCtrl_Confirm_BreakIn(LLONG lLoginId)
{
    NET_CTRL_CLEAR_ALARM stuInfo = {sizeof(stuInfo)};
    stuInfo.nChannelID = 0;                             // channel of door
    stuInfo.bEventType = TRUE;                          // set TRUE so as to enable nEventType filed
    stuInfo.nEventType = DH_ALARM_ACCESS_CTL_BREAK_IN;  // event type

    BOOL bRet = CLIENT_ControlDevice(lLoginId, DH_CTRL_CLEAR_ALARM, (char*)&stuInfo, SDK_API_WAITTIME);
    if (bRet)
    {
        printf("Confirm event at door %d with event BreakIn ok!\n", stuInfo.nChannelID);
    } 
    else
    {
        printf("Confirm event at door %d with event BreakIn failed:0x%08x\n", stuInfo.nChannelID, CLIENT_GetLastError());
    }
}
