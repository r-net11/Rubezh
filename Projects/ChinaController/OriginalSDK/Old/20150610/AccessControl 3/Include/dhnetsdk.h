...
#define DH_DEVSTATE_LIMIT_LOGIN_TIME	0x0054	// Query online overtime of device logging in and return a BTY£¨UNIT£ºMIN£© ,0 means no limite and The non-zero positive integer means restrictions on the number of minutes
...
#define DH_ALARM_DEVICE_MSG_NOTIFY        0x3199     // device to platform notice event¼þ(corresponding to  structure  ALARM_DEVICE_MSG_NOTIFY_INFO)
...
#define NET_ERROR_GETCFG_MULTICAST	_EC(52)	    // Failed to get config£ºmulticast port by channel
...
typedef struct tagNET_CTRL_SET_TEMPERATURE
{
    ...
    int                 nTemperature;                           // Temperature value(¡æ)
...
typedef struct tagNET_CTRL_AIRCONDITION_SETWINDMODE
{
    DWORD                       dwSize;
    char				        szDeviceID[DH_MAX_DEV_ID_LEN];	// SN 
    EM_AIRCONDITION_WINDMODE    emAirconditionWindMode;         // Fan modeÊ½
...
typedef enum _EM_NET_VEHICLE_TYPE
{
    ...
    NET_VEHICLE_TYPE_MOTORCYCLE,                        // "Motorcycle"Ä¦ÍÐ³µ        
...
typedef enum _EM_NET_RECORD_TYPE
{
    ...
    NET_RECORD_TRAFFICFLOW_STATE,                           // Search traffic flow mode£¬corresponding FIND_RECORD_TRAFFICFLOW_CONDITION
}EM_NET_RECORD_TYPE;
...
#define MAX_ALARMEXT_MODULE_NUM          8                  // max expansionmodule quantity
...
typedef struct tagTYRE_INFO_UNIT
{
    ...
    int                 nTemp;                           // Temperature,Unit¡æ
    int                 nTempLimit;                      // Temperature,Unit¡æ
...
typedef struct tagNET_CTRL_PLAYAUDIO_INFO
{
    ...
    char                szDetail[DH_COMMON_STRING_128]; // detailed content type detail£º
                                                        // Phrase type:"Welcome"
                                                        // File type: "/home/parking.pcm"
                                                        // PlateNumbe type: "ÕãA12345"
...
typedef struct 
{
    ...
    char				szText[128];					// the interrelated text of object,such as number plate,container number
                                                            // "ObjectType"Îª"Vehicle" or "Logo"£¨try to use Logo.Vehicle is used to be compatible with old product£©means logo£¬support£º
                                                            // "Unknown" Unknown 
                                                            // "Audi" Audi
                                                            // "Honda" Honda
                                                            // "Buick" Buick
...
typedef struct tagDEV_EVENT_TRAFFICGATE_INFO 
{
	...
	BYTE				bReserved[2500];				// Reserved bytes, leave extended
...
typedef struct __MEDIAFILE_FACE_DETECTION_INFO
{
    ...
    BYTE                nFileType;                                  // file type  1£ºjpg picture
...
typedef struct __NET_VIDEOANALYSE_STATE
{
	...
	char               szState[DH_COMMON_STRING_64]; // Channel Status,Running"£ºRun£¬"Stop"£ºStop£¬"NoStart"£ºNot started£¬"Failed"£ºfailed £¬"Successed"£ºsuccessed
...
typedef struct tagNET_VIDEODIAGNOSIS_COMMON_INFO
{
    ...
    char                            szResultAddress[DH_COMMON_STRING_128];  // Õï¶Ï½á¹û´æ·ÅµØÖ·
...
{
	DWORD                dwSize;                         // This structure size
	char*                 pszProjectNames;               // Program name to the name of the separator && as the example£ºProject1&&Project2   
...
typedef struct RESERVED_DATA_INTEL_BOX
{
    ...
    DWORD  dwInternalTime;      // picture upload interval time£¬unit£ºs
	BYTE   bReserved[1020];	 //Reserved
...
typedef struct tagNET_OUT_STORAGE_DEV_INFOS
{
    ...
    DH_STORAGE_DEVICE   stuStoregeDevInfos[MAX_DEVICE_VOLUME_NUMS]; //device info list,DH_STORAGE_DEVICEµÄdwsize need to assign value
...
typedef enum tagNET_SPLIT_OPERATE_TYPE
{
    ...
    NET_SPLIT_OPERATE_SET_PREPULLSRC,            // set pre stream srouce, corresponding to NET_IN_SPLIT_SET_PREPULLSRC ºÍ NET_OUT_SPLIT_SET_PREPULLSRC
    NET_SPLIT_OPERATE_SET_HIGHLIGHT,             // set source frame brightness switch, corresponding to NET_IN_SPLIT_SET_HIGHLIGHT ºÍ NET_OUT_SPLIT_SET_HIGHLIGHT
...
{
    NET_MONITORWALL_OPERATE_ADD,            // add tv wall, corresponding to  NET_IN_MONITORWALL_ADD ºÍ NET_OUT_MONITORWALL_ADD
    NET_MONITORWALL_OPERATE_CTRL_TOUR,      // scheme tour control, corresponding to  NET_IN_CTRL_COLLECTIONTOUR ºÍ NET_OUT_CTRL_COLLECTIONTOUR
    ...
    NET_MONITORWALL_OPERATE_GET_COLL_SCHD,  // get scheme schedule, corresponding to  NET_IN_MONITORWALL_GET_COLL_SCHD ºÍ NET_OUT_MONITORWALL_GET_COLL_SCHD
...
// set file property¡ªproperty name
...
typedef struct tagDH_IN_UPLOAD_REMOTE_FILE
{
    ...
    const char*         pszFolderDst;       // target file path£ºmay be NULL, NULL device use default path
...
typedef struct tagNET_RECORD_TRAFFIC_FLOW_STATE 
{
    ...
    float                           fBackOfQueue;               // line length£¬unit£ºmeter, from signal cross to front line end
...
{
    int                    nPositionX;           //PTZ horizontal motion position, effective range£º[0,3600]
    int                    nPositionY;           //PTZ vertical motion position, effective range£º[-1800,1800]
    int                    nZoom;                //PTZ aperture change position, the effective range£º[0,128]
    char                   szReserve[32];        //Reserved
}PTZ_SPACE_UNIT;
...
typedef struct tagNET_CB_RECORD_INFO
{
    ...
    DWORD                   dwType;                 // Video type,defined by bit as follows£º
                                                    // Bit0-Timing video 
...
typedef struct tagNET_IN_EXCHANGEDATA
{
    ...
    int             nCollectTime;                   // Serial collection time length(unit£ºms)
...
typedef struct tagNET_CB_BURNSTATE
{
    ...
    const char*         szState;                        // info type
                                                        //"UploadFileStart"£ºmay start to upload attachment
                                                        //"InitBurnDevice":initialize burning device
                                                        //"Burning":burning
                                                        //"BurnExtraFileStop"£ºstop burning
...
{
    DWORD                   dwSize;
    char                    szState[DH_MAX_NAME_LEN];      // "start"£ºsystem is ready,may start to upload; "busy"£ºsystem busy, try later."error"£ºsystem is not burning, return error, request failed
...
typedef enum tagNET_RAID_OPERATE_TYPE
{
    ...
    NET_RAID_OPERATE_MODIFY,                 // modify RAID, corresponding to  structure  NET_IN_RAID_MODIFY ºÍ NET_OUT_RAID_MODIFY
...
typedef struct tagNET_IN_ENCODE_CFG_CAPS
{
    DWORD               dwSize;           
    int                 nChannelId;                         // channel no.    
    int                 nStreamType;                        // stream type£¬0£ºmain stream£»1£ºsub stream1£»2£ºsub stream2£»3£ºsub stream3£»4£ºsnapshot stream
...
typedef struct tagNET_STREAM_CFG_CAPS
{
    ...
    BYTE                bH264ProfileRank[DH_PROFILE_HIGH];          // supportedµÄH.264 Profile level,refer to enumeration type EM_H264_PROFILE_RANK;  
...
{
    DWORD               dwStructSize;                               //  structure ´óÐ¡
    BYTE                bEventAction;                               // event action£¬0 means impulse event,1 means continuous event start,2 means continuous event end;
...
// get current sub system enable status(correspondingDH_DEVSTATE_GET_ALARM_SUBSYSTEM_ACTIVATESTATUSÃüÁî)
...
{
    NET_VIDEOJOIN_GET_STATUS,                   // get splicing status, corresponding to  NET_IN_VIDEOJOIN_GET_STATUS ºÍ NET_OUT_VIDEOJOIN_GET_STATUS
        ...
        NET_VIDEOJOIN_CALIBRATE,                // mark ,  corresponding to  NET_IN_VIDEOJOIN_CALIBRATE ºÍ NET_OUT_VIDEOJOIN_CALIBRATE
...
// cancel writing in remote device info£¬lAttachHandle is CLIENT_NetStorageAttachWriteInfoµÄ return value
...
// by file playback¡ªattached device IP, proxy use only
...
// by time playback¡ªattached device IP, proxy use only
...
// check bus route CLIENT_CheckBusLine() input parameter
typedef struct tagNET_IN_CHECK_BUS_LINE 
{
    DWORD           dwSize;
    char            szLineID[DH_COMMON_STRING_32];          // route mark
    char            szDesignation[DH_COMMON_STRING_32];     // route no.
    NET_LINE_DIRECTION  emDirection;                        // route direction
    int             nSiteCount;                             // route total stops
    char            szDirectionName[DH_COMMON_STRING_32];   // route direction name
    char            szVersion[DH_COMMON_STRING_32];         //route version no.
}NET_IN_CHECK_BUS_LINE;


// bus if update status type
typedef enum tagNET_BUS_LINE_UPDATE_TYPE
{
    NET_BUS_LINE_UPDATE_UNKNOWN = 0,
    NET_BUS_LINE_UPDATE_UPDATED,                            // need update
    NET_BUS_LINE_UPDATE_NOT_UPDATED,                        // not need update
}NET_BUS_LINE_UPDATE_TYPE;

// check bus route CLIENT_CheckBusLine() output parameter
typedef struct tagNET_OUT_CHECK_BUS_LINE
{
    DWORD           dwSize;
    char            szDeviceID[DH_COMMON_STRING_32];        // device ID
    char            szLineID[DH_COMMON_STRING_32];          // route mark
    NET_BUS_LINE_UPDATE_TYPE emUpdate;                      // if need to update status instruction
...
#define MAX_SITE_GPS_POINT_NUM                      64      // describe one bus stop max GPS points

// bus stop zone shape type
typedef enum tagNET_SITE_AREA_TYPE
{
    NET_SITE_AREA_ENCLOSURE = 0,                            // fense shape
...
typedef struct tagNET_BUS_SITE_INFO
{
    ...
    int             nSiteCount;                             // route total stops
    NET_LINE_DIRECTION  emDirection;                        // direction
    int             nStopSequence;                          // stop no.
    char            szSiteID[DH_COMMON_STRING_32];          // stop ID
    char            szSiteName[DH_COMMON_STRING_32];        // stop name
    NET_SITE_AREA_TYPE emAreaType;                          // stop zone type
    int             nGPSPointNum;                           // valid GPS point
    GPS_POINT       stuPoint[MAX_SITE_GPS_POINT_NUM];       // stop GPS info
    int             nRadius;                                // stop radius, unit: meter, only circular stop is valid
    int             nSpeedLimit;                            // stop speed limit, unit: km/H
    int             nDriveTime;                             // time from last stop tocurrent stop, unit :minute
    int             nTicketPrice;                           // ticket price
    DH_TSECT        stuPowerSchedule[DH_TSCHE_DAY_NUM][DH_TSCHE_SEC_NUM]; // boot up schedule, each element means Sunday, Saturday and holidays. 
}NET_BUS_SITE_INFO;

#define MAX_BUS_STOP_NUM                            32      // one bus route max stop number

// send bus route CLIENT_DispatchBusLineInfo() port input parameter
typedef struct tagNET_IN_DISPATCH_BUS_LINE_INFO 
{
    DWORD           dwSize;
    int             nBusStopNum;                            // bus route stop number
    NET_BUS_SITE_INFO   stuStopInfo[MAX_BUS_STOP_NUM];      // bus route info
}NET_IN_DISPATCH_BUS_LINE_INFO;

//send bus route CLIENT_DispatchBusLineInfo() port output parameter
