...
typedef struct 
{
    ...
    char				szText[128];					// the interrelated text of object,such as number plate,container number
                                                            // "ObjectType"为"Vehicle" or "Logo"（try to use Logo.Vehicle is used to be compatible with old product）means logo，support：
                                                            // "Unknown" Unknown 
                                                            // "Audi" Audi
                                                            // "Honda" Honda
                                                            // "Buick" Buick
...
// device capacity type, correspondingCLIENT_GetDevCaps port 
#define NET_DEV_CAP_SEQPOWER            0x01                // power sequence capacity, pInBuf=NET_IN_CAP_SEQPOWER*, pOutBuf=NET_OUT_CAP_SEQPOWER*
#define NET_ENCODE_CFG_CAPS             0x02                // device encode config corresponding capacity, pInBuf=NET_IN_ENCODE_CFG_CAPS*, pOutBuf= NET_OUT_ENCODE_CFG_CAPS*
...
typedef struct tagNET_STREAM_CFG_CAPS
{
    ...
    DH_RESOLUTION_INFO  stuResolutionTypes[DH_MAX_CAPTURE_SIZE_NUM];// supportedvideo resolution
    ...
    BYTE                bH264ProfileRank[DH_PROFILE_HIGH];          // supported的H.264 Profile level,refer to enumeration type EM_H264_PROFILE_RANK;  
...
{
    DWORD               dwStructSize;                               //  structure 大小
    BYTE                bEventAction;                               // event action，0 means impulse event,1 means continuous event start,2 means continuous event end;
...
// cancel writing in remote device info，lAttachHandle is CLIENT_NetStorageAttachWriteInfo的 return value
