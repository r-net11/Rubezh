...
namespace GKIntegratedTest
{
	...
	public partial class ItegratedTest
	{
		...
		{
			CheckTime(()=>RunProcess("RubezhService", "RubezhServerPath"), "Запуск сервера");
			for (int i = 1; i <= 10; i++)
			{
		...
		public void InitializeConfiguration()
		{
			...
			SafeRubezhService.GKCallbackResultEvent -= OnGKCallbackResult;
			SafeRubezhService.GKCallbackResultEvent += OnGKCallbackResult;
			SafeRubezhService.JournalItemsEvent -= OnNewJournalItems;
			SafeRubezhService.JournalItemsEvent += OnNewJournalItems;
		}
		...
		public void SetConfigAndRestartImitator()
		{
			...
			CheckTime(() => ClientManager.RubezhService.SetLocalConfig(), "Загрузка конфигурации на сервер");
		...
		SKDCard AddNewUser(int level, params GKDoor[] doors)
		{
			...
			if (organisation == null)
			{
				...
				result = ClientManager.RubezhService.SaveOrganisation(organisationDetails, true);
			...
			if (shortEmployee == null)
			{
				...
				result = ClientManager.RubezhService.SaveEmployee(employee, true);
				Assert.IsFalse(result.HasError, result.Error);
			}
			else
			{
			...
			result = ClientManager.RubezhService.AddCard(card, card.EmployeeName);
			Assert.IsFalse(result.HasError, result.Error);
			return card;
		}
	}
