...
namespace AutomationModule.ViewModels
{
	public class OpcDaClientReadWriteTagsViewModel : SaveCancelDialogViewModel
	{
		...
		void OnReadTag()
		{
			...
			WaitHelper.Execute(() =>
			{
				result = ClientManager.RubezhService.ReadOpcDaServerTags(RubezhServiceFactory.UID, OpcServer);
			});
		...
		void OnWriteTag()
		{
			...
			WaitHelper.Execute(() =>
			{
				...
				result = ClientManager.RubezhService.WriteOpcDaServerTag(
					RubezhServiceFactory.UID, SelectedTag.Uid, value); 
			});
		}
		bool CanWriteTag() 
		{ 
