...
namespace GKModule.Models
{
	public class DeviceCommandsViewModel : BaseViewModel
	{
		...
		void OnShowInfo()
		{
			var result = ClientManager.RubezhService.GKGetDeviceInfo(SelectedDevice.Device);
			if (result.HasError)
			{
		...
		{
			var result = ClientManager.RubezhService.GKSyncronyseTime(SelectedDevice.Device);
			if (result.HasError)
			{
				MessageBoxService.ShowWarning(result.Error);
				return;
		...
		void OnWriteConfig()
		{
			if (CheckNeedSave(true))
			{
				...
				{
					var result = ClientManager.RubezhService.GKWriteConfiguration(SelectedDevice.Device.GKParent);
					if (result.HasError)
					{
		...
		void OnReadConfiguration()
		{
			var thread = new Thread(() =>
			{
				...
				var result = ClientManager.RubezhService.GKReadConfiguration(SelectedDevice.Device);
		...
		void OnReadConfigFile()
		{
			...
			var result = ClientManager.RubezhService.GKReadConfigurationFromGKFile(SelectedDevice.Device);
			if (result.HasError)
			{
		...
		//			var result = ClientManager.RubezhService.GKUpdateFirmware(SelectedDevice.Device, firmwareBytes);
		...
		void OnAutoSearch()
		{
			var thread = new Thread(() =>
			{
				var result = ClientManager.RubezhService.GKAutoSearch(SelectedDevice.Device);
		...
		void OnGetUsers()
		{
			var result = ClientManager.RubezhService.GKGetUsers(SelectedDevice.Device);
			if (result.HasError)
			{
		...
		void OnRewriteUsers()
		{
			var result = ClientManager.RubezhService.GKRewriteUsers(SelectedDevice.Device.UID);
			if (result.HasError)
			{
		...
		void OnRewriteAllSchedules()
		{
			var thread = new Thread(() =>
			{
				var result = ClientManager.RubezhService.GKRewriteAllSchedules(SelectedDevice.Device);
