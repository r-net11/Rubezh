	...
	public static class ClientSettings
	{
		...
		static void LoadArchiveDefaultState()
		{
			if (File.Exists(ArchiveDefaultStateFileName))
			{
				...
				{
					var xmlSerializer = XmlSerializer.FromTypes(new[] {typeof (ArchiveDefaultState)})[0]; //new XmlSerializer(typeof(ArchiveDefaultState));
		...
		static void LoadSKDSettings()
		{
			if (File.Exists(SKDSettingsFileName))
			{
				...
				{
					var xmlSerializer = XmlSerializer.FromTypes(new[] {typeof (SKDSettings)})[0]; //new XmlSerializer(typeof(SKDSettings));
