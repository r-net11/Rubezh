...
{
	class GKPumpStationPainter : BaseZonePainter<GKPumpStation, ShowGKPumpStationEvent>
	{
		...
		public GKPumpStationPainter(PresenterItem presenterItem)
			: base(presenterItem)
		{
			_textDrawing = null;
			_scaleTransform = new ScaleTransform();
			if (Item != null && presenterItem.Element is ElementRectangleGKPumpStation)
			{
				var elementRectangleGKPumpStation = presenterItem.Element as ElementRectangleGKPumpStation;
				_showState = elementRectangleGKPumpStation.ShowState;
				_showDelay = elementRectangleGKPumpStation.ShowDelay;
			}
			else if (Item != null && presenterItem.Element is ElementPolygonGKPumpStation)
			{
				var elementPolygonGKPumpStation = presenterItem.Element as ElementPolygonGKPumpStation;
				this._showState = elementPolygonGKPumpStation.ShowState;
				this._showDelay = elementPolygonGKPumpStation.ShowDelay;
			}
		}

		protected override GKPumpStation CreateItem(PresenterItem presenterItem)
		{
			var element = presenterItem.Element as IElementPumpStation;
			return element == null ? null : PlanPresenter.Cache.Get<GKPumpStation>(element.ItemUID);
		}
		protected override StateTooltipViewModel<GKPumpStation> CreateToolTip()
		{
			return new PumpStationTooltipViewModel(Item);
		}
		...
		{
			return new PumpStationDetailsViewModel(Item);
		}
