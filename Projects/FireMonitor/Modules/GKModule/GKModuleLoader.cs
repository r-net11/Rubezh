...
namespace GKModule
{
	public partial class GKModuleLoader : ModuleBase, IReportProviderModule, ILayoutProviderModule
	{
		...
		{
			var direction = XManager.Directions.FirstOrDefault(x => x.BaseUID == directinUID);
			if (direction != null)
			{
		...
		{
			var mpt = XManager.MPTs.FirstOrDefault(x => x.BaseUID == mptUID);
			if (mpt != null)
			{
		...
		{
			var pumpStation = XManager.PumpStations.FirstOrDefault(x => x.BaseUID == pumpStationUID);
			if (pumpStation != null)
			{
		...
		{
			var delay = XManager.Delays.FirstOrDefault(x => x.BaseUID == delayUID);
		...
		{
			var pim = XManager.AutoGeneratedPims.FirstOrDefault(x => x.BaseUID == pimUID);
			if (pim != null)
			{
		...
		public IEnumerable<ILayoutPartPresenter> GetLayoutParts()
		{
			...
			yield return new LayoutPartPresenter(LayoutPartIdentities.GKJournals, "Журнал событий", "Book.png", (p) => JournalsViewModel);
			yield return new LayoutPartPresenter(LayoutPartIdentities.GKArchive, "Архив", "Archive.png", (p) => ArchiveViewModel);
		}
