...
namespace SKDModule.Employees.Models
{
	public sealed class ReportSettings : BaseViewModel
	{
		...
		private bool _isEnabledPaperKindSettings;
		private int _width;
		private int _height;
		...
		public bool IsEnabledPaperKindSettings
		{
			get { return _isEnabledPaperKindSettings; }
			set
			{
				if (_isEnabledPaperKindSettings == value) return;
				_isEnabledPaperKindSettings = value;
				OnPropertyChanged(() => IsEnabledPaperKindSettings);
			}
		}
		...
		public ReportSettings(Guid organisationUID)
		{
			...
			this.WhenAny(x => x.SelectedPaperKindSetting, x => x.Value)
				.Subscribe(value =>
				{
					if (value is UserKindSetting)
						IsEnabledPaperKindSettings = true;

					IsEnabledPaperKindSettings = false;
				});
		}
