	...
	public class SKDModuleLoader : ModuleBase, IReportProviderModule, ILayoutProviderModule, ISKDReportProviderModule
	{
		...
		public override IEnumerable<NavigationItem> CreateNavigation()
		{
			return new List<NavigationItem>
				{
				new NavigationItem("ÑÊÄ", "SKDW",
					new List<NavigationItem>()
					{
						new NavigationItem<ShowSKDDeviceEvent, Guid>(DevicesViewModel, "Óñòðîéñòâà", "Tree", null, PermissionType.Oper_Strazh_Devices_View, Guid.Empty) {IsVisible = SKDManager.Devices.Count > 1},
						new NavigationItem<ShowSKDZoneEvent, Guid>(ZonesViewModel, "Çîíû", "Zones", null, PermissionType.Oper_Strazh_Zones_View, Guid.Empty) {IsVisible = SKDManager.Zones.Count > 0},
						new NavigationItem<ShowSKDDoorEvent, Guid>(DoorsViewModel, "Òî÷êè äîñòóïà", "DoorW", null, PermissionType.Oper_Strazh_Doors_View, Guid.Empty) {IsVisible = SKDManager.Doors.Count > 0},
						new NavigationItem<ShowHREvent>(HRViewModel, "Êàðòîòåêà", "Kartoteka2W"),
						new NavigationItem("Ó÷åò ðàáî÷åãî âðåìåíè", "TimeTrackingW", new List<NavigationItem>()
						{
							new NavigationItem<ShowTimeIntervalsEvent, Guid>(DayIntervalsViewModel, "Äíåâíûå ãðàôèêè", "ShedulesDaylyW", null, PermissionType.Oper_SKD_TimeTrack_DaySchedules_View, Guid.Empty),
							new NavigationItem<ShowWeeklyIntervalsEvent, Guid>(ScheduleSchemesViewModel, "Ãðàôèêè", "SheduleWeeklyW", null, PermissionType.Oper_SKD_TimeTrack_ScheduleSchemes_View, Guid.Empty),
							new NavigationItem<ShowHolidaysEvent, Guid>(HolidaysViewModel, "Ñîêðàù¸ííûå äíè", "HolidaysW", null, PermissionType.Oper_SKD_TimeTrack_Holidays_View, Guid.Empty),
							new NavigationItem<ShowShedulesEvent, Guid>(SchedulesViewModel, "Ãðàôèêè ðàáîò", "ShedulesW", null, PermissionType.Oper_SKD_TimeTrack_Schedules_View, Guid.Empty),
							new NavigationItem<ShowTimeTrackingEvent>(TimeTrackingViewModel, "Ó÷åò ðàáî÷åãî âðåìåíè", "TimeTrackingW", null, PermissionType.Oper_SKD_TimeTrack_Report_View),
						}),
					})
				};
		}
