...
#define CFG_CMD_BATTERY_LOW_POWER   "BatteryLowPowerAlarm"      // 电池电量低配置(CFG_BATTER_LOW_POWER_INFO)
...
typedef struct tagCFG_MOTION_WINDOW
{
	...
	CFG_RECT             stuWindow;            // dynamic window area, coordinate range [0, 8192)
}CFG_MOTION_WINDOW;
...
typedef struct tagCFG_MOTION_INFO 
{
	...
	int                  nSubRatio;                                 //dynamic inspection of residual error threshold, 1-100
...
typedef struct tagCFG_CAP_VIDEOINPUT_INFO
{
	...
	int             nWideDynamicRange;						// support wide dynamic range or not, 0-not support, 1- support digital wid dynamic
...
#define AV_CFG_Max_TV_In_Block		64			// TV max amount in the zone
...
typedef struct tagCFG_STREAM_ENCRYPT_INFO
{
	...
	WORD						wEncryptLenth;		// The key length, the current is AES algorithm type， which means that key bits(currently support 128,192,256 three kinds, such as: wEncryptLenth was : wEncryptLenth为128，the key information in ENCRYPT_KEY_INFO byAesKey [0] to [15]) )
...
typedef struct tagCFG_VIDEOIN_EXPOSURE_BASE
{
    ...
    BYTE        byExposureSpeed;        // Exposure level of speed: 0 - Auto Exposure 1 - Exposure Level 1,2 - Exposure Level 2 ... n-1 the maximum exposure level, n the time limit with automatic exposure, n +1 custom time manual exposure (n means support the exposure level)
...
typedef struct tagCFG_ANALOGALARM_INFO	// =>CFG_CMD_ANALOGALARM
{
	...
	float				fLowerLimit;								// The lower limit range 
	...
	int                 nLevel1;                                    // 第一级级联地址, 表示连接在第nSlot串口上的第nLevel1个探测器或仪表, 从0开始
...
typedef struct tagCFG_ALARM_SUBSYSTEM_INFO 
{
	...
	CFG_TIME_SECTION	stuTime[WEEK_DAY_NUM][MAX_REC_TSECT];// 有效时间段
